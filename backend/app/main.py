...
# Shared SQLite Connection Helper with WAL & Busy Timeout
def get_db():
...
# 6. Physical Malkhana QR Verification
...
async def verify_qr_endpoint(
    ...
    if not decoded_data:
        raise HTTPException(status_code=400, detail="No readable QR code pattern detected in the uploaded image.")

    parts = decoded_data.split("|")
    if len(parts) < 3:
        raise HTTPException(status_code=422, detail=f"Malformed NyayaVault QR payload: '{decoded_data}'")
    ...
    if not row:
        raise HTTPException(status_code=404, detail=f"Document UUID '{doc_id}' decoded from QR, but no on-chain ledger record exists.")
...
# 7. Content Verification Endpoint
